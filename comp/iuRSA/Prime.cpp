...
static int __fastcall SmallFactor(NN_DIGIT *, unsigned int);
static int __fastcall FermatTest(NN_DIGIT *, unsigned int);
...
/* Generates a probable prime a between b and c such that a-1 is
   ...
   unsuccessful.
 */
int __fastcall GeneratePrime(NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, NN_DIGIT *d, unsigned int digits, R_RANDOM_STRUCT *randomStruct)
//NN_DIGIT *a, *b, *c, *d;
//unsigned int digits;
//R_RANDOM_STRUCT *randomStruct;
{
   ...
   */
  if ((bool)(status = R_GenerateBytes (block, digits * NN_DIGIT_LEN, randomStruct)))
    return (status);
 ...
 */
static int __fastcall ProbablePrime(NN_DIGIT *a,unsigned int aDigits)
{
...
/* Returns nonzero iff a has a prime factor in SMALL_PRIMES.
   ...
   Assumes aDigits < MAX_NN_DIGITS.
 */
static int __fastcall SmallFactor(NN_DIGIT *a, unsigned int aDigits)
{
  int status;
  NN_DIGIT t[1];
  unsigned int i;
...
/* Returns nonzero iff a passes Fermat's test for witness 2.
   ...
   Assumes aDigits < MAX_NN_DIGITS.
 */
static int __fastcall FermatTest(NN_DIGIT *a,unsigned int aDigits)
{
  int status;
  NN_DIGIT t[MAX_NN_DIGITS], u[MAX_NN_DIGITS];
