...
static int __fastcall RSAFilter(NN_DIGIT *, unsigned int, NN_DIGIT *, unsigned int);
static int __fastcall RelativelyPrime(NN_DIGIT *, unsigned int, NN_DIGIT *, unsigned int);

/* Generates an RSA key pair with a given length and public exponent.
 */
int __fastcall R_GeneratePEMKeys (//publicKey, privateKey, protoKey, randomStruct)
...
R_RANDOM_STRUCT *randomStruct)//;                          /* random structure */
{
  ...
  do {
    if (bool(status = GeneratePrime (p, t, u, v, pDigits, randomStruct)))
      return (status);
  }
  ...
  NN_ASSIGN_DIGIT (v, 2, pDigits);
  do {
    if (bool(status = GeneratePrime (q, t, u, v, pDigits, randomStruct)))
      return (status);
  }
 ...
 */
static int __fastcall RSAFilter (NN_DIGIT *a, unsigned int aDigits, NN_DIGIT *b, unsigned int bDigits)
//NN_DIGIT *a, *b;
 ...
 */
static int __fastcall RelativelyPrime (NN_DIGIT *a, unsigned int aDigits, NN_DIGIT *b, unsigned int bDigits)
//NN_DIGIT *a, *b;
//unsigned int aDigits, bDigits;
{
  int status;
