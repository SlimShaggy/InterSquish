...
static void __fastcall Unpack(unsigned char *, UINT4 *);
static void __fastcall Pack(UINT4 *, unsigned char *);
static void __fastcall DESKey(UINT4 *, unsigned char *, int);
static void __fastcall CookKey(UINT4 *, UINT4 *, int);
static void __fastcall DESFunction(UINT4 *, UINT4 *);

/* Initialize context.  Caller must zeroize the context when finished.
 */
void __fastcall DES_CBCInit (context, key, iv, encrypt)
...
/* DES-CBC block update operation. Continues a DES-CBC encryption
   ...
   the context.
 */
int __fastcall DES_CBCUpdate (context, output, input, len)
DES_CBC_CTX *context;                                            /* context */
...
void __fastcall DES_CBCRestart (context)
DES_CBC_CTX *context;
{
 ...
 */
void __fastcall DESX_CBCInit (context, key, iv, encrypt)
...
/* DESX-CBC block update operation. Continues a DESX-CBC encryption
   ...
   the context.
 */
int __fastcall DESX_CBCUpdate (context, output, input, len)
...
void __fastcall DESX_CBCRestart (context)
DESX_CBC_CTX *context;
{
  /* Reset to the original IV */
  context->iv[0] = context->originalIV[0];
...
/* Initialize context.  Caller must zeroize the context when finished.
 */
void __fastcall DES3_CBCInit(context, key, iv, encrypt)
...
int __fastcall DES3_CBCUpdate (context, output, input, len)
DES3_CBC_CTX *context;                                           /* context */
unsigned char *output;                                      /* output block */
unsigned char *input;                                        /* input block */
unsigned int len;                      /* length of input and output blocks */
...
void __fastcall DES3_CBCRestart (context)
...
static void __fastcall Pack (into, outof)
UINT4 *into;
unsigned char *outof;
{
  *into    = (*outof++ & 0xffL) << 24;
...
static void __fastcall Unpack (into, outof)
unsigned char *into;
UINT4 *outof;
{
...
static void __fastcall DESKey (subkeys, key, encrypt)
UINT4 subkeys[32];
unsigned char key[8];
int encrypt;
{
...
static void __fastcall CookKey (subkeys, kn, encrypt)
UINT4 *subkeys;
UINT4 *kn;
int encrypt;
{
...
static void __fastcall DESFunction (block, subkeys)
UINT4 *block;
UINT4 *subkeys;
{
